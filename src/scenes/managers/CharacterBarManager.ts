...
export interface CharacterBarHost {
  ...
  characterTooltip: Phaser.GameObjects.Container | null;
  enemyInfoWindow: Phaser.GameObjects.Container | null;
...
export class CharacterBarManager implements CharacterSlotManager {
  ...
  constructor(host: CharacterBarHost & Phaser.Scene) {
    this.host = host;
  }
  ...
  private closeCharacterTooltip(): void {
    const h = this.host;
    if (!h.characterTooltip) return;
    h.tweens.add({
      targets: h.characterTooltip,
      alpha: 0,
      duration: 100,
      ease: 'Sine.easeIn',
      onComplete: () => {
        h.characterTooltip?.destroy();
        h.characterTooltip = null;
      },
    });
  }
  ...
  closeEnemyInfoWindow(): void {
    const h = this.host;
    if (!h.enemyInfoWindow) return;
    h.tweens.add({
      targets: h.enemyInfoWindow,
      alpha: 0,
      duration: 100,
      ease: 'Sine.easeIn',
      onComplete: () => {
        h.enemyInfoWindow?.destroy();
        h.enemyInfoWindow = null;
      },
    });
  }
