...
export interface SkillContext {
  ...
  singleCard?: {
    ...
    index?: number;
    /**
     * 额外结算次数（0 = 基础结算，≥1 = 还需额外结算几次）。
     * 「单牌伤害额外结算一次」类效果（四象印白虎、李清照「豪放」、戚继光「荡寇」等）
     * 通过 `extraSettlements += 1` 声明，由 DamageSettlementManager 在同一张牌的放大动画
     * 结束、分数累加后，再完整播放一次结算循环（重新触发单牌结算技能，多个效果叠加）。
     * 为避免额外结算再被「额外结算类」技能递归叠加，这类技能自身在
     * `isExtraSettlement === true` 时不应再累加次数。
     */
    extraSettlements?: number;
    /**
     * 当前结算轮是否为「额外结算」（true = 由白虎/豪放/荡寇等产生的补充结算轮）。
     * 单牌结算技能（如罗成「舞枪」、薛万彻「骁锐」）在额外结算轮同样触发；
     * 但「额外结算一次」类效果自身据此不再递归累加次数。
     */
    isExtraSettlement?: boolean;
  };
  /**
